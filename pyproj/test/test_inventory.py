...
from dev.inventory import MobileInventory, InsufficientException
import pytest
...
class TestingInventoryCreation:
    ...
    def test_creating_inventory_with_list(self):
        with pytest.raises(TypeError):
            assert MobileInventory(["iPhone Model X", "Xiaomi Model Y", "Nokia Model Z"] ) == "Input inventory must be a dictionary"
    ...
    def test_creating_inventory_with_numeric_keys(self):
        with pytest.raises(ValueError):
            assert MobileInventory({1: "iPhone Model X", 2: "Xiaomi Model Y", 3: "Nokia Model Z"} ) == "Mobile model name must be a string"
    ...
    def test_creating_inventory_with_nonnumeric_values(self):
        with pytest.raises(ValueError):
            assert MobileInventory({"iPhone Model X": "100", "Xiaomi Model Y": "1000", "Nokia Model Z": "25"}) == "No. of mobiles must be a postive integer"

    # Define a pytest test method **'test_creating_inventory_with_negative_value'**, which checks if the  method raises a ValueError with the message "No. of mobiles must be a positive integer" using assert, when the dict {"iPhone Model X":'100', "Xiaomi Model Y": '1000', "Nokia Model Z": '25'} is passed as input.
    def test_creating_inventory_with_negative_value(self):
        with pytest.raises(ValueError):
            assert MobileInventory({"iPhone Model X": -45, "Xiaomi Model Y": 2000, "Nokia Model Z": 24}) == "No. of mobiles must be a postive integer"
...
class TestInventoryAddStock:
    ...
    # Define a pytest test method **'test_add_new_stock_as_dict'**, which adds the new stock {"iPhone Model X":50, "Xiaomi Model Y": 2000, "Nokia Model A":10} to the existing inventory, and update the **balance_inventory** attribute. Also, check if the updated **balance_inventory** equals {"iPhone Model X":100, "Xiaomi Model Y": 1000, "Nokia Model Z": 25} using assert.
    ...
    def test_add_new_stock_as_list(self):
        with pytest.raises(TypeError):
            assert TestInventoryAddStock.inventory.add_stock(["iPhone Model X", "Xiaomi Model Y", "Nokia Model Z"] ) == "Input stock must be a dictionary"
    ...
    def test_add_new_stock_with_numeric_keys(self):
        with pytest.raises(ValueError):
            assert TestInventoryAddStock.inventory.add_stock({1: "iPhone Model A", 2: "Xiaomi Model B", 3: "Nokia Model C"}) == "Mobile model name must be a string"
    ...
    def test_add_new_stock_with_nonnumeric_values(self):
        with pytest.raises(ValueError):
            assert TestInventoryAddStock.inventory.add_stock({"iPhone Model A": "50", "Xiaomi Model B": "2000", "Nokia Model C": "25"}) == "No. of mobiles must be a positive integer"
    ...
    def test_add_new_stock_with_float_values(self):
        with pytest.raises(ValueError):
            assert TestInventoryAddStock.inventory.add_stock({"iPhone Model A": 50.5, "Xiaomi Model B": 2000.3, "Nokia Model C": 25}) == "No. of mobiles must be a positive integer"
...
class TestInventorySellStock:
    ...
    def test_sell_stock_as_list(self):
        with pytest.raises(TypeError):
            assert TestInventorySellStock.inventory.sell_stock(["iPhone Model A", "Xiaomi Model B", "Nokia Model C"]) == "Requested stock must be a dictionary"
    ...
    def test_sell_stock_with_numeric_keys(self):
        with pytest.raises(ValueError):
            assert TestInventorySellStock.inventory.sell_stock({1: "iPhone Model A", 2:" Xiaomi Model B", 3: "Nokia Model C"}) == "Mobile model name must be a string"
    ...
    def test_sell_stock_with_nonnumeric_values(self):
        with pytest.raises(ValueError):
            assert TestInventorySellStock.inventory.sell_stock({"iPhone Model A": "2", "Xiaomi Model B": "3", "Nokia Model C": "4"}) == "No. of mobiles must be a positive integer"
    ...
    def test_sell_stock_with_float_values(self):
        with pytest.raises(ValueError):
            assert TestInventorySellStock.inventory.sell_stock({"iPhone Model A": 2.5, "Xiaomi Model B": 3.1, "Nokia Model C": 4}) == "No. of mobiles must be a positive integer"
    ...
    def test_sell_stock_of_nonexisting_model(self):
        with pytest.raises(InsufficientException):
            assert TestInventorySellStock.inventory.sell_stock({"iPhone Model B": 2, "Xiaomi Model B": 5}) == "No Stock. New Model Request"
    ...
    def test_sell_stock_of_insufficient_stock(self):
        with pytest.raises(InsufficientException):
            assert TestInventorySellStock.inventory.sell_stock({"iPhone Model A": 2, "Xiaomi Model B": 5, "Nokia Model C": 15}) == "Insufficient Stock"
